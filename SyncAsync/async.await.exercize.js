async function getInfo() {
  ...
  const movie = await getTopMovie(customer.id);
  const email = await sendEmail(movie.movieID);
...
function getCustomer(id) {
  ...
  const customers = [
    ...
    { id: 2, name: "Choi Seok Jun", isGold: true, email: "email" }
  ];
...
function getTopMovie(movieID) {
  console.log("Movie를 찾는중 입니다....");
  const movies = [
    { movieID: 1, content: "movie1" },
    { movieID: 2, content: "movie2" }
  ];
  return new Promise((resolve, reject) => {
    setTimeout(() => {
